...
You can customize the name of the metamodels using the Metamodel annotation elements.
...
But note that the NativeSql DSL doesn't support associating entities.
...
.. note::
    ...
    If the right-hand operand is ``null``, the WHERE or HAVING clause doesn't include the operator.
